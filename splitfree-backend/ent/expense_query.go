...
func (eq *ExpenseQuery) QueryDebtors() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, expense.DebtorsTable, expense.DebtorsPrimaryKey...),
		)
...
func (eq *ExpenseQuery) loadDebtors(ctx context.Context, query *UserQuery, nodes []*Expense, init func(*Expense), assign func(*Expense, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(expense.DebtorsTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(expense.DebtorsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(expense.DebtorsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(expense.DebtorsPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "debtors" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
