...
type ExpenseQuery struct {
	...
	withPaidBy  *UserQuery
	withDebtors *UserQuery
	withFKs     bool
...
// QueryPaidBy chains the current query on the "paid_by" edge.
func (eq *ExpenseQuery) QueryPaidBy() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, expense.PaidByTable, expense.PaidByColumn),
		)
...
// QueryDebtors chains the current query on the "debtors" edge.
func (eq *ExpenseQuery) QueryDebtors() *UserQuery {
	query := (&UserClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(expense.Table, expense.FieldID, selector),
			sqlgraph.To(user.Table, user.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, expense.DebtorsTable, expense.DebtorsColumn),
		)
		fromU = sqlgraph.SetNeighbors(eq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (eq *ExpenseQuery) Clone() *ExpenseQuery {
	...
	return &ExpenseQuery{
		...
		withPaidBy:  eq.withPaidBy.Clone(),
		withDebtors: eq.withDebtors.Clone(),
...
// WithPaidBy tells the query-builder to eager-load the nodes that are connected to
// the "paid_by" edge. The optional arguments are used to configure the query builder of the edge.
func (eq *ExpenseQuery) WithPaidBy(opts ...func(*UserQuery)) *ExpenseQuery {
	query := (&UserClient{config: eq.config}).Query()
...
// WithDebtors tells the query-builder to eager-load the nodes that are connected to
// the "debtors" edge. The optional arguments are used to configure the query builder of the edge.
func (eq *ExpenseQuery) WithDebtors(opts ...func(*UserQuery)) *ExpenseQuery {
	query := (&UserClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withDebtors = query
	return eq
}
...
func (eq *ExpenseQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Expense, error) {
	...
	if query := eq.withPaidBy; query != nil {
		if err := eq.loadPaidBy(ctx, query, nodes, nil,
			func(n *Expense, e *User) { n.Edges.PaidBy = e }); err != nil {
			return nil, err
		}
	}
	if query := eq.withDebtors; query != nil {
		if err := eq.loadDebtors(ctx, query, nodes,
			func(n *Expense) { n.Edges.Debtors = []*User{} },
			func(n *Expense, e *User) { n.Edges.Debtors = append(n.Edges.Debtors, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *ExpenseQuery) loadPaidBy(ctx context.Context, query *UserQuery, nodes []*Expense, init func(*Expense), assign func(*Expense, *User)) error {
...
func (eq *ExpenseQuery) loadDebtors(ctx context.Context, query *UserQuery, nodes []*Expense, init func(*Expense), assign func(*Expense, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(expense.DebtorsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.expense_debtors
		if fk == nil {
			return fmt.Errorf(`foreign-key "expense_debtors" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "expense_debtors" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
